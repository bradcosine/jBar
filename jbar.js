...
;(function ($) {
	...
	$.jBar = function(options) {
	    ...
	    // jBar Styles
	    var styles = '<style>' + '.jbar{background:' + backgroundColor + ';top:0;left:0;right:0;z-index:999998;color:#FFF;border-bottom:3px solid ' + borderColor + ';display:none;}'
	    		   + '.jbar-cta{display:block;max-width:1280px;margin:0 auto;padding:5px;}'
	    		   + '.jribbon{padding:5px 2px 0;z-index:999999;top:0;right:4%;display:none;width:38px;text-align:center;background:#DF5A02;border:3px solid #FFF;'
	    		   + 'border-top:none;box-shadow:0 2px 5px #333;color:#FFF;cursor:pointer;border-radius:0 0 3px 3px;}' 
	    		   + '.downarrow{top:11px;right:4%;width:50px;text-align:center;position:absolute;}' + '.jribbon:hover{background:#DF5A02;cursor:pointer;}' 
		...
		if (type == 'fixed') {
			...
			// Get jBar height for animation height
			var height = $('#jbar').outerHeight();
			
			// Initial Animation
	        $('body').prepend('<div id="jbar-push" style="height:'+ height +'px;display:none;" />');
